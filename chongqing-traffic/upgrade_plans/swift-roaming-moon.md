...
1.单文件单体：所有逻辑混在一个函数里，难以维护和扩展
2.功能单一：仅支持矩形查询，高德 API 还支持圆形、道路名查询
3.无 CLI：坐标硬编码在 `main()` 中，每次换区域要改代码
4.用 print() 代替日志：无法控制输出级别
5.无类型标注：代码可读性和 IDE 提示不足
6.错误处理不完整：无 HTTP 状态码检查、无超时、无重试
7.无测试：改动代码无法验证是否破坏现有功能
8.地图功能基础：无图例、无统计面板
